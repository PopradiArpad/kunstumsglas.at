...
const entity = (state=defaultState,action) => {
  switch (action.type) {
    ...
    case LOAD_ENTITY:             return loadEntity(state,action);
    case BACK:                    return back(state);
    case SAVE_ENTITY:             return saveEntity(state,action);
...
const loadEntity = (state,action) => {
  ...
  if (action.identity.id==='temporary') {
    newState = merge({},state,{_creationContext:{parentIdentity:action.parentIdentity,parentProperty:action.parentProperty}})
  }

  return startProcess(newState,LoadEntityProcess,action.identity,state.entityOverviews);
}
...
const back = (state) => {
  ...
  return lastIdentityInTheHistory
         ? startProcess(state,LoadEntityProcess,lastIdentityInTheHistory,state.entityOverviews)
         : state;
}

const saveEntity = (state,action) => {
  if (action.entityDescriptionDiff.identity.id==='temporary') {
    return startProcess(state,
...
const removeEntity = (state,action) => {
  ...
  return startProcess(state,RemoveEntityProcess, action.identity, getLastIdentityInTheHistory(state));
}
...
const processFinishedOk = (state,action) => {
  ...
  switch (state._runningProcess.name) {
    case LoadEntityProcess.processName:     return processOkLoadEntity(state,action);
    case SaveEntityProcess.processName:     return processOkSaveEntity(state);
    case CreateEntityToProcess.processName: return processOkCreateEntityTo(state);
    case RemoveEntityProcess.processName:   return processOkRemoveEntityProcess(state);
  }
}
...
const processFinishedError = (state,action) => {
  ...
  dispatchInNextTick(new AddError(state._runningProcess.name, action.errorMessages));
  return merge({},state,{_runningProcess:null});
}

const processOkLoadEntity = (state,action) => {
  let _history    = mkHistoryAfterLoadEntity(state.entityDescription,
                                             state._history,
  ...
  if (state._forcePicReloadEntityId && entityOverviews && entityOverviews[state._forcePicReloadEntityId]) {
...
const mkHistoryAfterLoadEntity = (currentEntityDescription,_history, newIdentity) => {
  ...
  let backward = (lastIdentityInTheHistory && lastIdentityInTheHistory.id===newIdentity.id);

  if (backward)
    history.pop();
  else {
    if (currentEntityDescription && (currentEntityDescription.identity.id !== newIdentity.id))
...
const startProcess = (state,processConstructor,p1,p2,p3,p4) => {
