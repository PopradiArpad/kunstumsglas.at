...
const auth = (state=defaultState, action) => {
  switch (action.type) {
    //from components
    case IS_LOGGED_IN:            return isLoggedIn(state);
    case LOGIN:                   return login(state,action);
    case LOGOUT:                  return logout(state);
    case SIGNUP:                  return signup(state,action);
    case SET_NEW_PASSWORD:        return setNewPassword(state,action);
...
const setNewPassword = (state,action) => {
  return startProcess(state,SetNewPasswordProcess,action.password,action.newPassword);
}
...
const processFinishedOk = (state,action) => {
  ...
  switch (state._runningProcess.name) {
    case IsLoggedInProcess.processName:      return processOkIsLoggedIn(state,action);
    case LogoutProcess.processName:          return processOkLogout(state,action);
    case LoginProcess.processName:           return processOkLogin(state,action);
    case SignupProcess.processName:          return processOkSignup(state);
    case SetNewPasswordProcess.processName:  return processOkSetNewPassword(state,action);
  }
}
...
const startProcess = (state,processConstructor,p1,p2) => {
