...
import getPropertyStateConstructor    from './propertyStates/getPropertyStateConstructor';
...
class EntityView extends Component {
  ...
  mkPropertyStates(props) {
    ...
    return entityDescription.propertyDescriptions.map((propertyDescription,ix)=>
                                                       this.mkPropertyState(entityDescription,
                                                                            propertyDescription,
                                                                            entityOverviews,
                                                                            ix));
  }
  ...
  mkPropertyState(entityDescription,propertyDescription,entityOverviews,ix) {
    const Constructor = getPropertyStateConstructor(propertyDescription.__typename);
    ...
    const args = {
       ...
       onCreateNewEntity: (dbModelName) => this.props.onCreateNewEntity(identity,property,dbModelName),
       entityDescription
    };
  ...
  setChanged(property,ix) {
    const changedPropertiesMap = merge({},this.state.changedPropertiesMap,{[property]:ix});
    ...
    while(true) { // eslint-disable-line no-constant-condition
      ...
      if (nextExternalComponentState) {
        if (nextExternalComponentState.isFocusable())
          ...
          return this.setState({focusedPropertyIx:0});
    }
  }
  ...
  getControlPanel() {
    const entityChanged = Object.keys(this.state.changedPropertiesMap).length > 0;
    ...
    return  (<div className="kugm-entityview-buttons">
              {allBack}
              {remove}
              {save}
            </div>);
  ...
  getAllBackButton() {
    return (
      <div className="kugm-entityview-buttons-allback" key={'allBack'} onClick={this.setAllPropertyBack}>
        &#8592;
      </div>);
  }
  ...
  getRemoveButton() {
    return (
      <div className="kugm-entityview-buttons-remove" key={'remove'} onClick={this.askUserAboutRemoving}>
        &#9747;
      </div>);
  }
  ...
  getSaveButton() {
    return (
      <div className="kugm-entityview-buttons-save" key={'save'} onClick={this.saveEntity}>
        Speichern
      </div>);
  }
  ...
  getPropertyViews() {
    ...
    return this.state.propertyStates.map((ecs,ix)=>ecs.render(ix,ix===focusedPropertyIx));
  }
  ...
  askUserAboutRemoving(e) {
    ...
    this.setState({dialog:<ConfirmRemoveDialog name={name}
                                               dbModel={dbModel}
                                               onCancel={()=> this.setState({dialog:null})}
                                               onRemove={()=>{this.setState({dialog:null});this.props.onRemoveEntity(identity);}}/>});
  }
