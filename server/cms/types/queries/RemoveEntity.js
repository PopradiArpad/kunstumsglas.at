...
function RemoveEntity(db) {
  this.db = db;
}

RemoveEntity.prototype = Object.create(Object.prototype);

RemoveEntity.prototype.run = function(root, {identity,parentIdentity}) {
  let entity;
  let parentEntity;
  let property;
  ...
  return getEntity(identity)
         .then(e=>{entity=e;
           if (! isEntityRemovable(entity))
             return Promise.reject(new Error(`Cannot remove ${identity}: it's not removable`));
         })
         ...
         .then(e=>{
           ...
           if (! property)
             return Promise.reject(new Error(`Cannot remove ${identity}: it's supposed parent ${parentIdentity} doesn't referres to it`));
         })
         //I don't know why the direct setting doesn't work
         .then(()=>getModel(parentEntity).update({_id:parentEntity._id},{$pull: {[property]: entity._id}}))
...
const getPropertyInWhichEntityIsReferred = (parentEntity,entity) => {
  ...
  schema.eachPath(property=>{
    ...
    if (parentEntity[property].find(id=>id.toString()===entity._id.toString()))
...
const removeRelatedGfsFiles = (db,entity) => {
  ...
  schema.eachPath(property=>{
    if (getPropertyTypeName(schemaPaths[property].options)==='FileUploadStatus')
      fileProperties.push(property);
  });

  return Promise.all(fileProperties.map(p=>removeRelatedGfsFileIfExist(db,entity,p)));
}
