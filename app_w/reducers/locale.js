...
const locale = (state=defaultState,action) => {
  switch (action.type) {
    ...
    case SET_NEXT_PRODUCT_GROUP:     return setNextProductGroup(state,action);
    case SET_PREVIOUS_PRODUCT_GROUP: return setPreviousProductGroup(state,action);
...
const setLocale = (state,action) => {
  ...
  if (localizedMessages || landingPageData) {
    ...
    if (localizedMessages)
      newState = merge(newState,{locale:action.locale},{localizedMessages});

    if (landingPageData)
      newState = merge(newState,{locale:action.locale},{landingPageData});
...
const setNextProductGroup = (state,action) => {
  const {productGroupIx,productGroupIds}  = getProductGroupIx(state,action.currentPgId);
  const nextProductGroupIx = (productGroupIx<productGroupIds.length-1) ? productGroupIx+1 : 0;
...
const setPreviousProductGroup = (state,action) => {
  const {productGroupIx,productGroupIds}  = getProductGroupIx(state,action.currentPgId);
  const nextProductGroupIx = (productGroupIx>0) ? productGroupIx-1 : productGroupIds.length-1;
  const nextProductGroupId = productGroupIds[nextProductGroupIx];
  pushBrowserHistoryToProductGroup(nextProductGroupId);
  return state;
}
...
const processFinishedOk = (state,action) => {
  ...
  switch (state._runningProcess.name) {
    case LoadLocaleProcess.processName:     return processOkLoadLocale(state,action);
  }
}
