...
const productGroup = (state = defaultState, action) => {
  switch (action.type) {
    case SET_PRODUCT_GROUP:
      return setProductGroup(state, action);
    case SET_PRODUCT:
      return setProduct(state, action);
    case SET_LOCALE:
      return setLocale(state, action);
    case SET_NEXT_PRODUCT:
      return setNextProduct(state);
    case SET_PREVIOUS_PRODUCT:
      return setPreviousProduct(state);
...
const setProductGroup = (state, action) => {
  ...
  if (productGroupLocale && productGroupLocale[state._locale])
    return merge(
      ...
      state,
      { productGroup: null },
      { productGroup: productGroupLocale[state._locale] }
    );

  return startProcess(
    state,
    LoadProductGroupProcess,
    action.id,
    state._locale
  );
};
...
const processFinishedOk = (state, action) => {
  ...
  switch (state._runningProcess.name) {
    case LoadProductGroupProcess.processName:
      return processOkLoadProductGroup(state, action);
    case LoadProductProcess.processName:
      return processOkLoadProduct(state, action);
  }
};
...
const processOkLoadProduct = (state, action) => {
  ...
  if (
    !state.productGroup ||
    state.productGroup.id !== state._runningProcess.pgid ||
    state.productGroup.locale !== state._locale
  )
